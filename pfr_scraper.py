...
if __name__ == "__main__":
    if not API_KEY:
        ...
        # 1. Get Team Standings (Offense/Defense)
                ...
                for conference in standings_data:
                    for team_info in conference:
                        all_teams_stats.append({
                            ...
                            'W': team_info['won'], 'L': team_info['lost'], 'T': team_info['drawn'],
        ...
        # 2. Get Player Stats for current and previous year
            ...
            try:
                ...
                if all_players_stats:
                    ...
                    for player in all_players_stats:
                        ...
                        for group in player['statistics']:
                            if group['group'] == 'Passing':
                                passing_list.append({**p_info, **group['statistics']})
                            elif group['group'] == 'Rushing':
                                rushing_list.append({**p_info, **group['statistics']})
                            elif group['group'] == 'Receiving':
                                receiving_list.append({**p_info, **group['statistics']})
                    
                    prefix = "" if year_to_fetch == YEAR else f"{year_to_fetch}_"
                    write_to_sheet(spreadsheet, f"{prefix}O_Player_Passing", pd.DataFrame(passing_list))
                    write_to_sheet(spreadsheet, f"{prefix}O_Player_Rushing", pd.DataFrame(rushing_list))
                    write_to_sheet(spreadsheet, f"{prefix}O_Player_Receiving", pd.DataFrame(receiving_list))
            except Exception as e:
                print(f"❌ Could not process Player Stats for {year_to_fetch}: {e}")

        # 3. Scrape Depth Charts
